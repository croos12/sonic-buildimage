...
log "Starting unsigned build step (bfb + bin)"
START_BUILD=$(date +%s)
...
log "Starting signed build step (bfb + bin)"
START_SIGNED=$(date +%s)
...
echo "Unsigned bfb+bin: ${BUILD_DURATION}s ($(date -ud @${BUILD_DURATION} +%H:%M:%S))" | tee -a build_timing.log
echo "Signed bfb+bin  : ${SIGNED_DURATION}s ($(date -ud @${SIGNED_DURATION} +%H:%M:%S))" | tee -a build_timing.log
